...
用户反馈
推荐优化
基于标注文献的关键词自动拓展（反馈学习闭环，无人工审核）：

- 高分标注（Must Read / 5★ / Save）论文 → LLM 从摘要提炼新词 → 自动写入 learned 词表（与用户手配的 keywords 分离，带权重）
- 防漂移护栏：同一新词需在 ≥2 篇高分论文中出现才提权；词权重随时间衰减
- 低分标注（Not Relevant）→ 只对相关词降权，不自动写入 exclude（避免单次误标误杀整个方向）
- 不微调模型：每日反馈信号太稀疏，采用相关反馈式词表更新，透明、可回滚、留审计日志
- 下游检索与打分读取 learned 词表即生效，无需改动代码
